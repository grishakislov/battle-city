...
internal class WorldCollisionLayer extends Sprite {
    ...
    public function initialize(levelId:uint):void {
        _visual = BattleStageLoader.drawStageToBitmap(levelId, BattleStageDrawMode.MAIN);
        _data = Game.getLevelDataManager().getLevelDataByID(levelId);
        addChild(_visual);
    ...
    public function applyDestruction(worldPoint:Point, direction:TankDirection):void {
        ...
        var currentTile:String = BattleStageLoader.getTileByID(_data.getDataAt(mapPoint.x, mapPoint.y)).getName();
...
//                    deleteCollisionAt(state.coords);
    ...
    private function getTile(name:String):TileAsset {
        return Game.getAssetManager().getTileAsset(name);
    }
